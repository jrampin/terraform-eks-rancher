resource "aws_autoscaling_policy" "cpu_up" {
  name = format("%s-nodes-cpu-scale-up", var.project_name)
...
resource "aws_cloudwatch_metric_alarm" "cpu_up" {
  ...
  alarm_name = format("%s-nodes-cpu-high", var.project_name)
...
resource "aws_autoscaling_policy" "cpu_down" {
  name = format("%s-nodes-cpu-scale-down", var.project_name)
  adjustment_type = "ChangeInCapacity"
...
resource "aws_cloudwatch_metric_alarm" "cpu_down" {
  ...
  alarm_name = format("%s-nodes-cpu-low", var.project_name)
